...
def get_cancer_outcome(
        previous_cancer_volume, chemo_dosage, radio_dosage,
        rho, K, beta_c, alpha, beta):
    cancer_volume = previous_cancer_volume * (1 + \
                                  + rho * np.log(K / previous_cancer_volume) \
                                  - beta_c * chemo_dosage \
                                  - (alpha * radio_dosage + beta * radio_dosage ** 2))  # add noise to fit residuals
    return cancer_volume

def get_chemo_dosage(current_chemo_dose, previous_chemo_dose, drug_half_life = 1):

    chemo_dosage = previous_chemo_dose * np.exp(-np.log(2) / drug_half_life) + current_chemo_dose
    return chemo_dosage
...
def simulate(simulation_params, num_time_steps, assigned_actions=None):
    ...
    chemo_instant_dosage = np.zeros((num_patients, num_time_steps))
    ...
    for i in range(num_patients):
        ...
        for t in range(1, num_time_steps):
            cancer_volume[i, t] = get_cancer_outcome(
                cancer_volume[i, t - 1], chemo_dosage[i, t - 1],
                radio_dosage[i, t - 1], rho, K, beta_c, alpha, beta
            ) + cancer_volume[i, t - 1]*noise[t]
            ...
            # Update chemo dosage
            chemo_dosage[i, t] = get_chemo_dosage(current_chemo_dose, previous_chemo_dose, drug_half_life)
            chemo_dosage[i, t] = previous_chemo_dose * np.exp(-np.log(2) / drug_half_life) + current_chemo_dose
            chemo_instant_dosage[i,t] = current_chemo_dose
    ...
    outputs = {'cancer_volume': cancer_volume,
               ...
               'chemo_instant_dosage': chemo_instant_dosage,
...
def get_simulate_df(outputs):
    ...
    # Iterate through each feature (key-value pair) in the input dictionary
    for feature_name_raw, feature_array in cancer_volume_data.items():
        # Keep the feature name as is for the column header
        feature_column_name = feature_name_raw # This line is changed

        # Create a temporary DataFrame from the current feature's array
        # Rows are patients, columns are time points
        temp_df = pd.DataFrame(feature_array, columns=[f'Time_{i}' for i in range(feature_array.shape[1])])

        # Add Patient_ID
        temp_df['Patient_ID'] = [f'Patient_{i}' for i in range(temp_df.shape[0])]

        # Melt the temporary DataFrame to long format: Patient_ID, Time_Point, Feature_Value
        melted_temp_df = temp_df.melt(
            id_vars=['Patient_ID'],
            var_name='Time_Point_Str', # Use a temporary name for time string
            value_name=feature_column_name # Use the original feature name here
        )

        # Convert 'Time_Point_Str' to integer 'Time_Point'
        melted_temp_df['Time_Point'] = melted_temp_df['Time_Point_Str'].str.replace('Time_', '').astype(int)
        melted_temp_df = melted_temp_df.drop(columns=['Time_Point_Str']) # Drop the string column

        # If this is the first feature, initialize merged_df with it
        if merged_df is None:
            merged_df = melted_temp_df
        else:
            merged_df = pd.merge(merged_df, melted_temp_df, on=['Patient_ID', 'Time_Point'], how='outer')
    return merged_df
